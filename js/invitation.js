...
function setupGalleryRail(urls) {
    ...
    const rail = track ? track.closest('.gallery-rail') : null;
    ...
    track.classList.remove('gallery-track--single', 'gallery-track--marquee-js');
    ...
    if (!urls || urls.length === 0) {
        ...
        placeholder.id = 'gallery-empty';
        placeholder.innerHTML = '<strong>Pronto</strong> agregaremos fotos aquí desde el panel.';
    ...
    const items = urls.map((url) => {
        ...
        return `
            <div class="gallery-item">
                <img src="${safe}" alt="Foto" loading="lazy" decoding="async" />
            </div>
        `;
    }).join('');
    ...
    // Una sola foto: NO duplicar; bucle suave con CSS
    if (urls.length === 1 || prefersReduced) {
        track.innerHTML = items;
        track.classList.add('gallery-track', 'gallery-track--single');
        return;
    }

    // Varias fotos: duplicamos la secuencia solo para un carril infinito sin “huecos”
    track.innerHTML = items + items;
    track.classList.add('gallery-track', 'gallery-track--marquee-js');
    ...
    function measureLoopWidth() {
        ...
        if (!loopWidth || loopWidth < 40) loopWidth = Math.max(track.scrollWidth, 400);
    }
    ...
    function speedPx() {
        return mobileMq.matches ? 0.55 : 1.85;
    }
    ...
    function tick() {
        ...
        galleryRafId = requestAnimationFrame(tick);
    }
