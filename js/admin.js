// admin.js - Panel de Control de Invitaciones (Versión con Admin desde Firebase)
...
async function getEventStats() {
    try {
        ...
        guestsSnapshot.forEach(doc => {
            const data = normalizeGuestRecord(doc.data() || {});
            stats.total++;
            stats.totalCupos += data.maxGuests || 0;
            
            if (data.status === 'confirmed') {
                stats.confirmed++;
                stats.confirmedTotal += data.confirmedGuests || 0;
...
function updateStatsUI(stats) {
    ...
    if (statElements.confirmedTotal) {
        const confirmedPersons = stats.confirmedTotal || 0;
        const totalCupos = stats.totalCupos || 0;
        statElements.confirmedTotal.textContent = totalCupos
            ? `${confirmedPersons} / ${totalCupos} cupos`
            : `${confirmedPersons} personas`;
    }
...
async function loadGuests() {
    try {
        ...
        const result = await getAllGuests(currentPage, 20, searchQuery, currentStatusFilter);
...
async function getAllGuests(page = 1, limit = 20, search = '', statusFilter = 'all') {
    try {
        ...
        if (search) {
            ...
            const guests = applyStatusFilter(Array.from(guestsMap.values()), statusFilter);
            ...
            return {
                ...
                pagination: {
                    ...
                    limit,
                    total: guests.length,
                    totalPages: Math.max(1, Math.ceil(guests.length / limit)),
        ...
        // Sin búsqueda: cargar y paginar localmente (evita repetir página 1)
        ...
        const filtered = applyStatusFilter(allGuests, statusFilter);
        const total = filtered.length;
        const totalPages = Math.max(1, Math.ceil(total / limit));
        const safePage = Math.min(Math.max(1, page), totalPages);
        const startIndex = (safePage - 1) * limit;
        const endIndex = startIndex + limit;
        const guests = filtered.slice(startIndex, endIndex);
        ...
        if (error.code === 'failed-precondition') {
            ...
            filteredGuests = applyStatusFilter(filteredGuests, statusFilter);
            ...
            return {
                ...
                pagination: {
                    ...
                    totalPages: Math.max(1, Math.ceil(filteredGuests.length / limit)),
...
function updateGuestsTable(guests) {
    ...
    guests.forEach(guest => {
        const normalized = normalizeGuestRecord(guest || {});
        const statusClass = normalized.status || 'pending';
        const statusText = getStatusText(normalized.status);
        const maxGuests = normalized.maxGuests || 2;
        const confirmed = normalized.confirmedGuests || 0;
...
function normalizeGuestRecord(guest) {
    ...
    const maxGuests = Number(normalized.maxGuests || 0) || 0;

    // Compatibilidad con datos viejos: deducir estado por attendance.
    if (!normalized.status && normalized.attendance) {
        normalized.status = normalized.attendance === 'yes' ? 'confirmed' : 'declined';
    }
    if (!normalized.status && normalized.lastResponse?.attendance) {
        normalized.status = normalized.lastResponse.attendance === 'yes' ? 'confirmed' : 'declined';
    }
    if (!normalized.status) normalized.status = 'pending';

    // Compatibilidad de número confirmado.
    let confirmed = Number(normalized.confirmedGuests || 0) || 0;
    if (!confirmed && normalized.status === 'confirmed') {
        const lrGuests = Number(normalized.lastResponse?.guestsCount || 0) || 0;
        const oldGuests = Number(normalized.guestsCount || 0) || 0;
        confirmed = lrGuests || oldGuests || 1;
    }
    if (maxGuests > 0) confirmed = Math.min(confirmed, maxGuests);
    normalized.confirmedGuests = confirmed;

    return normalized;
}

function applyStatusFilter(guests, statusFilter) {
    if (!Array.isArray(guests)) return [];
    if (!statusFilter || statusFilter === 'all') return guests;
    return guests.filter((g) => normalizeGuestRecord(g).status === statusFilter);
}
...
async function handleNewGuestSubmit(e) {
    ...
    try {
        ...
        console.log("Creando invitación:", { name, maxGuests, customCode });
        
        const result = await createGuestInvitation(name, maxGuests, customCode);
        
        showToast('Invitación creada exitosamente', 'success');
        
        // Resetear formulario y luego mostrar enlace generado + conteo inicial
        resetNewGuestForm();
        const createdConfirmed = result?.data?.confirmedGuests ?? 0;
        const createdMaxGuests = result?.data?.maxGuests ?? maxGuests ?? 0;
        showCreatedInvitation(result.invitationCode, result.link, createdConfirmed, createdMaxGuests);
...
async function createGuestInvitation(name = '', maxGuests = 2, customCode = '') {
    try {
        if (!firebaseDb) throw new Error('Firebase no disponible');
...
function showCreatedInvitation(code, link, confirmedGuests = 0, maxGuests = 0) {
    const createdSection = document.getElementById('created-invitation');
    const linkInput = document.getElementById('invitation-link');
    const copyBtn = document.getElementById('btn-copy-link');
    const confirmedEl = document.getElementById('created-confirmed-count');
    const maxGuestsEl = document.getElementById('created-max-guests-count');
    ...
    if (confirmedEl) confirmedEl.textContent = String(confirmedGuests ?? 0);
    if (maxGuestsEl) maxGuestsEl.textContent = String(maxGuests ?? 0);
...
async function handleExportCSV() {
    try {
        ...
        link.setAttribute('download', `invitados-invitacion-${new Date().toISOString().split('T')[0]}.csv`);
...
async function handleBulkImportConfirm() {
    ...
    try {
        ...
        for (const line of lines) {
            ...
            if (parts.length >= 2) {
                const inferredMaxGuests = parts.length >= 3 ? parseInt(parts[2], 10) : parseInt(parts[1], 10);
                guestList.push({
                    name: parts[0],
                    maxGuests: inferredMaxGuests
                });
            }
        }
...
function initMobileMenu() {
    ...
    document.addEventListener('click', (e) => {
        ...
        const t = e.target;
        if (mobileNav.contains(t) || menuBtn.contains(t)) return;
        mobileNav.classList.remove('active');
        menuBtn.innerHTML = '<i class="fas fa-bars"></i>';
        menuBtn.setAttribute('aria-expanded', 'false');
    });
...
async function ensureEventConfigDocExists() {
    ...
    if (!doc.exists) {
        await ref.set({
            ...
            giftsText: 'Los mejores detalles hacen este día aún más especial',
            giftsSubtext: 'Tu puntual asistencia hace que todo sea perfecto',
...
async function loadEventConfigSettings() {
    try {
        ...
        setValue('cfg-gifts-sub', cfg.giftsSubtext);
...
async function saveEventConfigSettings() {
    try {
        ...
        const giftsSubtext = getValue('cfg-gifts-sub');
...
async function loadEventMediaSettings() {
    try {
        ...
        const suitUrl = String(data.dressSuitImageUrl || '').trim();
        const gownUrl = String(data.dressGownImageUrl || '').trim();
        const base = getBaseUrl();
        const defSuit = `${base}assets/dress-code-suit.svg`;
        const defGown = `${base}assets/dress-code-gown.svg`;
        const ps = document.getElementById('preview-dress-suit');
        const pg = document.getElementById('preview-dress-gown');
        if (ps) ps.src = suitUrl || defSuit;
        if (pg) pg.src = gownUrl || defGown;
...
function setupMediaButtons() {
    ...
    document.getElementById('btn-upload-dress-suit')?.addEventListener('click', () => uploadDressCodeImage('suit'));
    document.getElementById('btn-upload-dress-gown')?.addEventListener('click', () => uploadDressCodeImage('gown'));
    document.getElementById('btn-clear-dress-suit')?.addEventListener('click', () => clearDressCodeImage('suit'));
    document.getElementById('btn-clear-dress-gown')?.addEventListener('click', () => clearDressCodeImage('gown'));
}
...
async function uploadDressCodeImage(which) {
    try {
        if (!firebaseStorage) throw new Error('Firebase Storage no disponible');
        if (!firebaseDb) throw new Error('Firebase Firestore no disponible');

        const inputId = which === 'suit' ? 'media-dress-suit-file' : 'media-dress-gown-file';
        const field = which === 'suit' ? 'dressSuitImageUrl' : 'dressGownImageUrl';
        ...
        const fileName = `${Date.now()}_${(file.name || 'img').replace(/\s+/g, '_')}`;
        const path = `media/dress-code/${which}_${fileName}`;
        const ref = firebaseStorage.ref().child(path);
        ...
        if (input) input.value = '';
        showToast('✅ Imagen de vestimenta actualizada', 'success');
        await loadEventMediaSettings();
    } catch (error) {
        console.error('❌ Error subiendo imagen de vestimenta:', error);
...
async function clearDressCodeImage(which) {
    try {
        if (!firebaseDb) throw new Error('Firebase Firestore no disponible');
        const field = which === 'suit' ? 'dressSuitImageUrl' : 'dressGownImageUrl';
        ...
        showToast('Imagen predeterminada restaurada', 'success');
        await loadEventMediaSettings();
    } catch (error) {
        console.error('❌ Error al restablecer imagen:', error);
